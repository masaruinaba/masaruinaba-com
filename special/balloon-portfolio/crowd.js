...
const colorSets=[
  ['#087dc8','#f05237','#f7bf16','#08aa92','#e85994'],
  ['#f47c28','#167dcc','#f5c01e','#e85b9b','#0aaf9c'],
  ['#1575cc','#f5b51c','#eb6293','#9756cb','#0daa87'],
  ['#f15b42','#f6c41c','#0d8dce','#e45a9e','#23ad79'],
  ['#2079cb','#f57b31','#f4bd1a','#dd5796','#10a8ae'],
];
const palette=colorSets[sessionSeed%colorSets.length];
// Familiar silhouettes, evenly mixed with the playful flower and coil.
const kinds=['round','heart','triangle','square','star','oval','petal','coil'];
...
function makeMonster(i){
  const shapeIndex=i%kinds.length,kind=kinds[shapeIndex],root=new THREE.Group(),mat=material(palette[Math.floor(random()*palette.length)]);
  ...
  const shape=geometries[shapeIndex];if(!shape.data.noSeam)softenFilm(mat,i);const body=new THREE.Mesh(shape.body,mat),rim=new THREE.Mesh(shape.rim,mat);
...
function rebuild(){
  ...
  for(let i=0;i<Math.ceil(columns*rows*1.45);i++)makeMonster(i);
...
function resize(){
  ...
  const area=actors.reduce((sum,a)=>sum+Math.PI*a.radius*a.radius,0),fill=Math.sqrt(viewWidth*viewHeight*1.60/area);
...
function pose(a,dt,isMonster){
  ...
  a.root.rotation.set(0,0,a.angle+a.turn+a.kick*.06+a.floatAngle);
...
function deform(a,dt){
  ...
  a.press+=((pressing?1:0)-a.press)*(1-Math.exp(-14*dt));
  if(pressing){
...
function draw(dt){
  ...
  for(const a of actors)pose(a,dt,true);for(const a of ornaments)pose(a,dt,false);speech?.update(time,dt,actors,camera,reduced.matches,held);updateWork(camera);renderer.render(scene,camera);
...
function bounce(a){motionUntil=time+2.3;a.velocity=5;a.vx+=Math.cos(a.phase)*.7;a.vy+=Math.sin(a.phase)*.7;a.omega+=.35;a.face?.touch(0,.4);document.body.dataset.touches=String(+(document.body.dataset.touches||0)+1);wake();}
...
canvas.addEventListener('pointerdown',e=>{
  ...
  if(held){
    if(e.pointerType==='touch'&&e.pointerId!==held.id){held.points.set(e.pointerId,world);const points=[...held.points.values()];held.twistStart=Math.atan2(points[1].y-points[0].y,points[1].x-points[0].x);held.rotationStart=held.actor.angle;held.rotating=held.moved=true;canvas.setPointerCapture(e.pointerId);}return;
  }
  ...
  held={id:e.pointerId,actor:a,startX:e.clientX,startY:e.clientY,startAt:performance.now(),moved:false,rotating:e.shiftKey,rotationStart:a.angle,points:new Map([[e.pointerId,world]]),local,target:world,offset:new THREE.Vector3(world.x-a.px,world.y-a.py,0)};
  ...
  canvas.setPointerCapture(e.pointerId);if(!held.rotating)bounce(a);else motionUntil=time+2.3;document.body.dataset.held='true';wake();
});
canvas.addEventListener('pointermove',e=>{
  if(!camera)return;
  const gaze=pointerWorld(e);
  if(gaze)for(const a of actors){
    const near=Math.hypot(gaze.x-a.root.position.x,gaze.y-a.root.position.y)<a.radius+1.15;
    if(near){a.root.updateWorldMatrix(true,false);const local=a.root.worldToLocal(new THREE.Vector3(gaze.x,gaze.y,a.root.position.z));a.face.look(local.x,local.y);}
    else a.face.look(0,0);
  }
  wake();
  ...
  if(held.rotating){
    const p=[...held.points.values()];held.actor.angle=held.rotationStart+(p.length>1?Math.atan2(p[1].y-p[0].y,p[1].x-p[0].x)-held.twistStart:(e.clientX-held.startX)*.012);held.actor.turn=held.actor.omega=0;
...
function release(tap=false){
  if(held){motionUntil=time+2.3;if(tap&&!held.rotating&&!held.moved&&performance.now()-held.startAt<650&&held.actor.face){showWork(held.actor,speech,time,reduced.matches);}}
...
canvas.addEventListener('keydown',e=>{if(e.code==='Space'){e.preventDefault();actors.forEach(bounce);}});
...
async function init(){
  try{
    renderer=new THREE.WebGLRenderer({canvas,antialias:true});renderer.transmissionResolutionScale=.25;renderer.setClearColor('#dcd8d0');renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=.98;
    ...
    speech=createSpeech(scene);createWindControls(applyWind);resize();createColorMode(scene,renderer,()=>draw(0));window.addEventListener('resize',resize);status.hidden=true;document.body.dataset.ready='true';wake();
