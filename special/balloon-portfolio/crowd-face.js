...
export function createCrowdedFace(root,body,index,anchor){
  ...
  for(const side of [-1,1]){
    const g=part((anchor?.x||0)+side*(anchor?.052:.073),anchor?anchor.y+.025:.30),white=new THREE.Mesh(sphere,ivory),pupil=new THREE.Mesh(pupilSphere,ink);
  ...
  const mouth=part(anchor?.x||0,anchor?anchor.y-.045:.13);mouth.add(new THREE.Mesh(lipsGeometry,lip));const opening=new THREE.Mesh(openingGeometry,inner);opening.position.z=.002;mouth.add(opening);
  ...
  return {face,mouthPosition(){root.updateWorldMatrix(true,true);return mouth.getWorldPosition(new THREE.Vector3());},look(x,y){targetX=Math.max(-1,Math.min(1,x*.85));targetY=Math.max(-1,Math.min(1,y*.85));lastLook=performance.now();},speak(v){speech=v;},touch(){reaction=1;},update(t,dt,mood,pressed,paused,reduced){
    ...
    const follow=1-Math.exp(-7*dt);gazeX+=(targetX-gazeX)*follow;gazeY+=(targetY-gazeY)*follow;
    for(const eye of eyes){const pupil=eye.children[1];pupil.position.x=gazeX*.019;pupil.position.y=-.004+gazeY*.016;}
    const closure=Math.max(blink,pressed*.48);for(const eye of eyes)eye.scale.set(anchor?.60:1,(anchor?.60:1)*(1-closure*.85),anchor?.60:1);
    mouth.scale.setScalar(anchor?.48:.88);mouth.scale.y*=1+speech*.70+reaction*.08;
