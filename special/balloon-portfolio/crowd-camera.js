...
export async function createCameraFriends(video,onStatus){
  ...
  const friends=Array.from({length:9},(_,i)=>({x:25+(i%5)*70,y:25+Math.floor(i/5)*200,vx:(i%2?1:-1)*(16+i%3*3),vy:(i%3-1)*12+4,angle:i*.4,spin:(i%2?1:-1)*.1,r:20+i%3*3,color:['#f1c444','#ee779c','#43b5b3','#648fda','#ed9260'][i%5],squish:0}));
  ...
  function frame(now){
    ...
    for(const [i,p] of friends.entries()){
      p.vy+=Math.sin(now*.0011+i)*dt*6;
      if(face){p.vx+=(face.x-p.x)*dt*.12;p.vy+=(face.y-p.y)*dt*.10;}
      p.x+=p.vx*dt;p.y+=p.vy*dt;p.squish*=Math.exp(-7*dt);p.angle+=p.spin*dt;p.spin*=Math.exp(-.3*dt);
      if(face){const dx=p.x-face.x,dy=p.y-face.y,rx=face.rx+p.r,ry=face.ry+p.r,d=Math.hypot(dx/rx,dy/ry);
        if(d<1){const nx=dx/(rx*rx),ny=dy/(ry*ry),norm=Math.hypot(nx,ny)||1,ux=nx/norm,uy=ny/norm;
          ...
          const speed=p.vx*ux+p.vy*uy;if(speed<0){p.vx-=ux*(speed*1.2-5);p.vy-=uy*(speed*1.2-5);p.squish=.08;p.spin=(i%2?1:-1)*.15;document.body.dataset.faceBumps=String(+(document.body.dataset.faceBumps||0)+1);}
        }
      }
      ...
      const speed=Math.hypot(p.vx,p.vy);if(speed>45){p.vx*=45/speed;p.vy*=45/speed;}
      ctx.save();ctx.translate(p.x,p.y);ctx.rotate(p.angle);ctx.scale(1+p.squish,1-p.squish);
  ...
  onStatus('Wave your hand to send the balloons flying.');raf=requestAnimationFrame(frame);
