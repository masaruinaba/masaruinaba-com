...
export function makeCameraMovable(element){
  ...
  function snap(){
    if(mobile()){
      const pointerId=drag?.id;drag=null;element.dataset.dragging='false';
      ...
      element.dataset.corner='center';element.tabIndex=-1;element.setAttribute('aria-label','Camera preview');return;
    }
    element.tabIndex=0;element.setAttribute('aria-label','Camera preview. Drag to a corner. Arrow keys move between corners.');
    const p=cornerPosition(corner,element.offsetWidth||180,element.offsetHeight||135,innerWidth,innerHeight);element.style.left=p.x+'px';element.style.top=p.y+'px';element.style.right=element.style.bottom='auto';element.dataset.corner=corner;
  }
  element.addEventListener('pointerdown',e=>{if(mobile()||e.button!==0)return;e.preventDefault();const rect=element.getBoundingClientRect();drag={id:e.pointerId,x:e.clientX-rect.left,y:e.clientY-rect.top};element.dataset.dragging='true';element.setPointerCapture(e.pointerId);});
  ...
  element.addEventListener('keydown',e=>{if(mobile()||!e.key.startsWith('Arrow'))return;e.preventDefault();let [v,h]=corner.split('-');if(e.key==='ArrowUp')v='top';if(e.key==='ArrowDown')v='bottom';if(e.key==='ArrowLeft')h='left';if(e.key==='ArrowRight')h='right';corner=v+'-'+h;snap();});
