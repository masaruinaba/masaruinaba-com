...
let owner=null,animation=null,shownAt=0,speaker=null,frameIndex=-1,frameKey='',lastPlacement=null,lastViewport='';
export function showWork(actor,speech,time,reduced){
  const item=projects[actor.projectIndex%projects.length];owner?.face.speak(0);owner=actor;shownAt=performance.now();
  ...
  animation?.cancel();
  animation=bubble.animate([{opacity:0,transform:'translate(-50%, -50%) scale(.18) rotate(-9deg)'},{opacity:1,transform:'translate(-50%, -50%) scale(1.055) rotate(2deg)',offset:.7},{opacity:1,transform:'translate(-50%, -50%) scale(1) rotate(0deg)'}],{duration:matchMedia('(prefers-reduced-motion: reduce)').matches?0:480,easing:'cubic-bezier(.18,.8,.3,1)'});
  document.body.dataset.work=item.titleEn||item.title;
}
export function updateWork(camera){
  if(!owner||bubble.hidden)return;

  const position=owner.face.mouthPosition();position.x+=(position.x>0?1:-1)*(owner.baseSize*.45+.55);position.y+=owner.baseSize*.12+(matchMedia('(prefers-reduced-motion: reduce)').matches?0:Math.sin(performance.now()*.00085+owner.phase+.7)*.05);position.project(camera);
  ...
  const placement=placeWork({x:anchorX,y:anchorY-imageHeight/2},speaker?.bounds(),{width:innerWidth,height:innerHeight},208,imageHeight,lastPlacement);
  if(!placement){bubble.style.visibility='hidden';return;}
  lastPlacement=placement;
  const nextKey=[placement.width,imageHeight,frameIndex].join(':');
  if(nextKey!==frameKey){frameKey=nextKey;bubble.style.setProperty('--frame-mask',frameMask(placement.width,imageHeight*placement.width/208,frameKinds[frameIndex]));}
