...
export function createWindControls(onWind){
  ...
  let stopFriends=null,cameraStream=null,micStream=null,audio=null,analyser=null,previous=null,timer=0,cameraToken=0,micToken=0,smoothX=0,smoothY=0;
  ...
  timer=setInterval(()=>{
    ...
    if(cameraStream&&video.readyState>=2){
      ...
      if(previous){const motion=estimateMotion(previous,current,80,60);smoothX+=(motion.x-smoothX)*.35;smoothY+=(motion.y-smoothY)*.35;if(motion.strength>.15&&Math.hypot(smoothX,smoothY)>.6)onWind(smoothX/5,smoothY/5,motion.strength*.65);}
      previous=current;
    }
